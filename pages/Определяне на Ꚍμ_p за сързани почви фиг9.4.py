...
# Добавете тези нови импорти за PDF:
import os
import tempfile
import base64
...
def generate_pdf_report():
    try:
        ...
        table_data = [
            ...
            ["Диаметър D", f"{st.session_state.get('fig9_4_D', D):.2f}", "cm"],
            ["Брой пластове", f"{n}", ""],
        ...
        try:
            ...
            if point_on_esr_eo is not None:
                ...
                if x_orange is not None:
                    ...
                    # ВЕРТИКАЛНА ОРАНЖЕВА ЛИНИЯ (от оранжева точка до y=1.35)
                    plt.plot([x_orange, x_orange], [y_red, 1.35], 
                            '--', color='orange', linewidth=2,
                            label='Вертикална линия до y=1.35')
            ...
            ax_bottom.set_ylim(0, 1.35)
            
            # Тикчета за H/D (долна ос)
            hd_ticks = np.linspace(0, 2, 11)  # 0, 0.2, 0.4, ..., 2.0
            ...
            # Преобразуване: τμ/p = 0.20 * (H/D)
            taumu_p_ticks = [0.20 * (tick / 2) for tick in hd_ticks]
            ...
            plt.title('Номограма: τμ/p за сързани почви (фиг. 9.4)', fontsize=14)
        ...
        start_page = st.session_state.get("start_page_taumu_9_4", 1)
