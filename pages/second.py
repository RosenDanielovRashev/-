...
for i in range(1, n + 1):
    ...
    Ei_list.append(Ei)
    hi_list.append(hi)
# АКТУАЛИЗАЦИЯ: Вземане на Ed от Ee (земно основание)
st.markdown("---")
if len(Ei_list_full) > 0:  # Проверяваме дали има въведени пластове
    Ee = Ei_list_full[-1]  # Вземаме последния пласт като Ee (земно основание)
    Ed = Ee  # Задаваме Ed = Ee
    st.markdown(f"#### 🟢 Автоматично определено Ed (модул на земното основание Ee)")
    st.info(f"Ed = Ee = {Ed:.2f} MPa (взето от последния пласт в списъка)")
else:
    st.error("⚠️ Няма въведени пластове за определяне на Ee!")
    st.info("Моля, върнете се на предишната страница и добавете поне един пласт")
    st.stop()
