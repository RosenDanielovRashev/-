...
# АКТУАЛИЗАЦИЯ: Автоматично определяне на Ed според броя пластове
st.markdown("---")
if len(Ee_list_full) > n:
    Ed = Ee_list_full[n+1]  # Вземаме модула на следващия пласт
    st.markdown(f"#### 🟢 Автоматично определено Ed (модул на еластичност под пласта)")
    st.info(f"Ed = {Ed:.2f} MPa (взет от пласт {n+1} в списъка)")
else:
    st.error("⚠️ Няма достатъчно пластове за автоматично определяне на Ed!")
    st.info("Моля, върнете се на предишната страница и добавете повече пластове")
    st.stop()
