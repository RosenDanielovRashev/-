...
# Проверка дали final_Ed_list съществува и има достатъчно елементи
if "final_Ed_list" not in st.session_state or len(st.session_state.final_Ed_list) < 2:
    st.error("⚠️ Липсва стойност за Ed или final_Ed_list има по-малко от 2 елемента!")
    st.info("Моля, върнете се на първата страница и изчислете всички пластове")
    st.stop()

# Вземане на ВТОРАТА стойност от списъка (индекс 1)
Ed = st.session_state.final_Ed_list[1]  # ← Това е корекцията

# Показване на информация за Ed
st.markdown(f"""
#### 🟢 Стойност за Ed (модул на деформация на земното основание)
- Взета от **втория пласт** в списъка
- Ed = {Ed:.2f} MPa
""")
