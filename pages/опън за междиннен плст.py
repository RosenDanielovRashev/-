        ...
        def generate_pdf_report(layer_idx, results, D, sigma_r=None, sigma_final=None, manual_value=None, check_passed=None):
            ...
            for formula in formulas:
                try:
                    ...
                    png_path = formula_png_from_svg_or_fallback(formula)
                    pdf.image(png_path, x=15, w=180)
                    pdf.ln(8)  # Добавяне на малко място след формулата
                    os.unlink(png_path)  # Изтриване на временния файл
                except Exception as e:
                    # Fallback: показване като чист текст
            ...
            if layer_idx > 0:
                ...
                # Показване на формулата за Esr като изображение
                try:
                    numerator = " + ".join([f"{results['E_values'][i]} \cdot {results['h_values'][i]}" for i in range(layer_idx)])
                    denominator = " + ".join([f"{results['h_values'][i]}" for i in range(layer_idx)])
                    formula_text = fr"Esr = \frac{{{numerator}}}{{{denominator}}} = {round(results['Esr_r'])}"
                    ...
                    pdf.cell(0, 6, f"${formula_text}$", 0, 1)
                    pdf.set_font("DejaVu", "", 10)
            else:
                pdf.cell(col_widths_calc[0], 8, "Esr:", 0, 0)
            ...
            if sigma_final is not None:
                ...
                # Показване на формулата за σR като изображение
                try:
                    formula_text = fr"\sigma_R = 1.15 \cdot {p} \cdot {sigma_r} = {sigma_final:.3f} \, \text{{MPa}}"
                    png_path = formula_png_from_svg_or_fallback(formula_text)
                    pdf.image(png_path, x=15, w=180)
                    pdf.ln(5)
                    os.unlink(png_path)
                    ...
                    pdf.set_font("DejaVu", "", 8)
                    pdf.cell(0, 8, f"${formula_text}$", 0, 1)
                    pdf.set_font("DejaVu", "", 10)
