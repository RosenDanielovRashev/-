...
def render_formula_to_image_fallback(formula_text, fontsize=22, dpi=450):
    """
...
def generate_pdf_report_2():
    ...
    pdf.cell(0, 15, 'ОПЪН В МЕЖДИЕН ПЛАСТ', ln=True, align='C')
    pdf.set_font('DejaVu', 'I', 12)
    pdf.cell(0, 10, 'Определяне опънното напрежение в междиен пласт от пътнатата конструкция фиг.9.3', ln=True, align='C')
    pdf.ln(6)
    ...
    pdf.cell(0, 10, 'Данни за пластовете:', ln=True)
    
    col_widths = [40, 40, 40, 40]
    pdf.set_font('DejaVu', 'B', 10)
    pdf.set_fill_color(200, 220, 255)
    pdf.cell(col_widths[0], row_height, 'Пласт', border=1, align='C', fill=True)
    pdf.cell(col_widths[1], row_height, 'h (см)', border=1, align='C', fill=True)
    pdf.cell(col_widths[2], row_height, 'E (MPa)', border=1, align='C', fill=True)
    pdf.cell(col_widths[3], row_height, 'Ed (MPa)', border=1, align='C', fill=True)
    pdf.ln(row_height)
    ...
    for i in range(n):
        pdf.set_fill_color(245, 245, 245) if fill else pdf.set_fill_color(255, 255, 255)
        pdf.cell(col_widths[0], row_height, f"Пласт {i+1}", border=1, align='C', fill=True)
        pdf.cell(col_widths[1], row_height, f"{h_values[i]:.2f}", border=1, align='C', fill=True)
        pdf.cell(col_widths[2], row_height, f"{E_values[i]:.2f}", border=1, align='C', fill=True)
        pdf.cell(col_widths[3], row_height, f"{Ed_values[i]:.2f}", border=1, align='C', fill=True)
        pdf.ln(row_height)
        fill = not fill
    ...
    # 3. Изчисления за избрания пласт
    if layer_idx in st.session_state.layer_results:
        results = st.session_state.layer_results[layer_idx]
        ...
        # Добавяне на секцията с формули
        pdf.add_formula_section("Изчислителни формули:", formulas_section3, columns=2, col_width=95, img_width=85, row_gap=-3)

        # Крайно изчисление на σR
        if 'final_sigma_R' in st.session_state:
            axle_load = st.session_state.get("axle_load", 100)
            p = 0.620 if axle_load == 100 else 0.633 if axle_load == 115 else 0.0
            sigma_nom = st.session_state.get("final_sigma", 0)
            sigma_final = st.session_state.get("final_sigma_R", 0)
            
            formulas_final = [
                fr"\sigma_R^{{номограма}} = {sigma_nom:.3f} \, \text{{MPa}}",
                fr"p = {p:.3f} \, \text{{ (за осов товар {axle_load} kN)}}",
                fr"\sigma_R = 1.15 \times {p:.3f} \times {sigma_nom:.3f} = {sigma_final:.3f} \, \text{{MPa}}"
            ]
            pdf.add_formula_section("Крайно изчисление на σR:", formulas_final, columns=1, col_width=180, img_width=160, row_gap=-3)

    pdf.ln(5)

    # 4. Графика (ако има)
    if 'fig' in globals() and 'fig' in locals():
        pdf.set_font('DejaVu', 'B', 14)
        pdf.cell(0, 10, '4. Графика на изолиниите', ln=True)
    ...
    if 'final_sigma_R' in st.session_state and f'manual_sigma_{layer_idx}' in st.session_state.manual_sigma_values:
        ...
        sigma_to_compare = st.session_state.final_sigma_R
        check_passed = sigma_to_compare <= manual_value
        ...
        pdf.set_font('DejaVu', '', 10)
        for label, val in [
            ('Изчислено σR', f"{sigma_to_compare:.3f} MPa"),
    ...
    pdf.cell(0, 10, f'Генерирано на: {datetime.now().strftime("%Y-%m-%d %H:%M:%S")}', 0, 0, 'C')
...
if st.button("📄 Генерирай PDF отчет", key="pdf_button_2"):
        ...
        try:
            ...
            if pdf_bytes:
                ...
                with open(tmpfile.name, "rb") as f:
                    ...
                    download_link = f'<a href="data:application/octet-stream;base64,{base64_pdf}" download="open_v_mezhdien_plast_report.pdf">Свали PDF отчет</a>'
