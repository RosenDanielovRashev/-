...
if all_data_ready:
    ...
    with cols[0]:
        ...
        if st.button("📊 Изпрати към 'Ꚍμ_p (фиг9.6)'", type="primary", use_container_width=True, key="to_fig9_6"):
            ...
            st.page_link("pages/Определяне на Ꚍμ_p за несързани почви maxH/D=1.5 фиг9.6.py", label="Към Ꚍμ_p (фиг9.6)", icon="📈")
    ...
    with cols[1]:
        ...
        if st.button("📊 Изпрати към 'Ꚍμ_p (фиг9.7)'", type="primary", use_container_width=True, key="to_fig9_7"):
            ...
            st.page_link("pages/Определяне на Ꚍμ_p за несързани почви maxH/D=2 фиг9.7.py", label="Към Ꚍμ_p (фиг9.7)", icon="📈")
else:
...
def generate_pdf_report(include_main, include_fig94, include_fig96, include_fig97, include_tension, include_intermediate):
    ...
    if include_main:
        ...
        # Общи параметри
        pdf.cell(0, 8, f'Брой пластове: {st.session_state.num_layers}', 0, 1)
        pdf.cell(0, 8, f'D: {st.session_state.final_D} cm', 0, 1)
        pdf.cell(0, 8, f'Осова тежест: {st.session_state.axle_load} kN', 0, 1)
        ...
        pdf.cell(0, 8, 'Ei – Модул на еластичност на пласта', 0, 1)
        pdf.cell(0, 8, 'Ee – Модул на еластичност на повърхността на пласта', 0, 1)
        pdf.cell(0, 8, 'h – Дебелина на пласта', 0, 1)
        pdf.cell(0, 8, 'D – Диаметър на отпечатък на колелото', 0, 1)        
        ...
        if 'lambda_op_input' in st.session_state and 'lambda_zp_input' in st.session_state:
            ...
            pdf.set_font('DejaVu', 'B', 14)
            pdf.cell(0, 8, 'Топлинни параметри', 0, 1)
            pdf.set_font('DejaVu', '', 12)
            pdf.cell(0, 8, f'λоп = {lambda_op} kcal/mhg', 0, 1)
            pdf.cell(0, 8, f'λзп = {lambda_zp} kcal/mhg', 0, 1)
            pdf.cell(0, 8, f'm = λзп / λоп = {lambda_zp} / {lambda_op} = {m_value:.2f}', 0, 1)
            pdf.cell(0, 8, f'z₁ = {z1} cm (дълбочина на замръзване в открито поле)', 0, 1)
            pdf.cell(0, 8, f'z = z₁ * m = {z1} * {m_value:.2f} = {z_value:.2f} cm', 0, 1)
            pdf.ln(8)
            ...
            if all('h' in layer for layer in st.session_state.layers_data):
                ...
                pdf.cell(0, 8, f'R₀ = Σh / Σλ = {sum_h:.2f} / {sum_lambda:.2f} = {R0:.2f} cm', 0, 1)
                pdf.ln(8)
            
            # Проверка
            pdf.set_font('DejaVu', 'B', 14)
            pdf.cell(0, 8, 'Проверка на изискванията', 0, 1)
            pdf.set_font('DejaVu', '', 12)
