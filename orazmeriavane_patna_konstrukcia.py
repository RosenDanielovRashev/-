...
if all_data_ready:
    # Вземаме Ee на следващия пласт (който не е изчислен)
    # Ако имаме n пласта, земното основание е Ee на пласт n+1
    # За пласт n+1, Ee = Ed на пласт n
    last_layer = st.session_state.layers_data[-1]
    st.session_state.final_Ed = last_layer.get("Ed", 2700.0)  # Ed на последния пласт е Ee за земното основание
    
    st.info(f"ℹ️ Ed за земното основание е определено като: "
            f"Ee = {st.session_state.final_Ed:.2f} MPa (взето от Ed на последния пласт)")
