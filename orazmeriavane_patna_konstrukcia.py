...
# Избор на страници за експорт
pages_to_export = st.multiselect(
    "Изберете страници за включване в PDF отчета:",
    options=[
        "Оразмеряване на пътна конструкция",
        "Опън в покритието",
        "Ꚍμ_p (фиг9.4 - сързани почви)",
        "Ꚍμ_p (фиг9.5 - сързани почви)",
        "Ꚍμ_p (фиг9.6 - несързани почви)",
        "Ꚍμ_p (фиг9.7 - несързани почви)"
    ],
    default=["Оразмеряване на пътна конструкция"]
)

# Допълнителни опции за отчета
report_title = st.text_input("Заглавие на отчета", "Оразмеряване на пътна конструкция")
include_graphs = st.checkbox("Включване на графики", value=True)
include_tables = st.checkbox("Включване на таблици", value=True)

if st.button("🖨️ Генериране на PDF отчет", type="primary"):
